...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Semantic search over Fed speeches with optional Bedrock Q&A"
    )
    ...
    parser.add_argument(
        "--rebuild-index",
        action="store_true",
        help="Rebuild the FAISS index from speeches.json.",
    )
    parser.add_argument(
        "--top-k",
        type=int,
    ...
    artifact_paths = ensure_data(args.rebuild_index, args.chunker)

    if not args.query and not args.interactive:
        print(
            "Index is ready. Use --query or --interactive to search speeches."
        )
        return

    try:
        speeches = data_module.load_speeches(SPEECHES_PATH)
    except FileNotFoundError as exc:
        raise SystemExit(f"Missing speech data: {exc}") from exc
    ...
    if args.interactive:
        _interactive_loop(speeches, chunks, model, index, args.top_k)


def ensure_data(rebuild_index: bool, chunker_name: str):
    """Ensure FAISS index exists, building it if necessary."""
    artifact_paths = get_artifact_paths(DATA_DIR, chunker_name)

    # Check if speeches.json exists
    if not SPEECHES_PATH.exists():
        raise SystemExit(
            f"Speech data not found at {SPEECHES_PATH}. "
            "Please ensure data/speeches.json exists."
        )
    ...
    if rebuild_index or index_missing:
        if rebuild_index:
            print("Rebuilding FAISS index from speeches.json...")
        else:
            print("Building FAISS index from speeches.json...")
        speeches = data_module.load_speeches(SPEECHES_PATH)
...
def _run_single_query(
    ...
    *,
    ...
    _print_results(display_scores, display_indices, speeches, chunks)
...
def _interactive_loop(speeches, chunks, model, index, top_k: int) -> None:
    ...
    try:
        while True:
            ...
            _print_results(scores, indices, speeches, chunks)
    except KeyboardInterrupt:
        print("\nExiting interactive mode.")


def _print_results(scores, indices, speeches, chunks) -> None:
    print("-" * 60)
    for rank, (score, idx) in enumerate(zip(scores, indices), start=1):
        ...
        snippet_text = chunk.get("text", speech.content).strip()
        chunk_number = chunk.get("chunk_index", 0) + 1
        print(
            f"[{rank}] {speech.title} — {speech.speaker} "
            f"({speech.date}) [chunk {chunk_number}]"
        )
...
def _prepare_rag_segments(
    ...
    for score, idx in zip(scores, indices):
        ...
        if speech_idx < 0 or speech_idx >= len(speeches):
            continue
        speech = speeches[speech_idx]
        text = chunk.get("text", speech.content).strip()
        segments.append(
            RagSegment(
                title=speech.title,
